...
impl VideoFilter {
    ...
    unsafe fn alloc_filter(
        ...
        if filter_ctx.is_null() {
            bail!("cannot allocate filter {filter:?} (name = {name:?})");
        }
    ...
    pub fn init(
        ...
        let output_ctx = unsafe {
            ...
            ffmpeg::av_opt_set_array(
                filter_ctx as *mut _,
                c"pixel_formats".as_ptr(),
                ffmpeg::AV_OPT_SEARCH_CHILDREN as c_int,
                0,
                pix_fmts.len() as c_uint,
                ffmpeg::AV_OPT_TYPE_PIXEL_FMT,
            ...
            ffmpeg::av_opt_set_array(
                filter_ctx as *mut _,
                c"colorspaces".as_ptr(),
                ffmpeg::AV_OPT_SEARCH_CHILDREN as c_int,
                0,
                color_spaces.len() as c_uint,
                ffmpeg::AV_OPT_TYPE_INT,
            ...
            ffmpeg::av_opt_set_array(
                filter_ctx as *mut _,
                c"colorranges".as_ptr(),
                ffmpeg::AV_OPT_SEARCH_CHILDREN as c_int,
                0,
                color_ranges.len() as c_uint,
                ffmpeg::AV_OPT_TYPE_INT,
